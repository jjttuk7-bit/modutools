...
const seoData: Record<SubmitSeoToolId, SeoBlock> = {
  'pdf-mask': {
    ...
    privacyDisclaimer:
      '본 마스킹 서비스는 업로드 즉시 클라우드에 전송되어 수집되는 가공 방식과 달리, 100% 로컬 디바이스 웹 브라우저 안에서만 격리 실행됩니다. 주민번호가 담긴 원본 실체가 서류 정리함의 외부에 결코 공개 유출되지 않습니다.',
    faqList: [
      {
  ...
  'pdf-merge': {
    ...
    privacyDisclaimer:
      '서류 정리함 PDF 컴바이너 모듈은 전송 백엔드가 없는 WebAssembly 및 로컬 API 가상화 기법을 사용하여, 이용 시 기밀 소송 조항이나 성적 기록서가 다른 곳으로 일절 백업 저장되지 않습니다.',
    faqList: [
      {
      ...
      },
      {
        ...
        a: "용량이 제한 폭을 훌쩍 넘길 땐 서류 정리함의 또 다른 동반자인 '이미지 압축' 혹은 'PDF 페이지 뽑기'를 동원해 사전에 불필요한 고해상도 영역을 절삭하는 방책을 제안합니다.",
      },
    ],
    relatedTools: [
      { name: 'PDF 페이지 뽑기', path: '/submit/pdf-extract', icon: 'Scissors' },
  ...
  'pdf-extract': {
    ...
    privacyDisclaimer:
      '절단 필터링 동작 시 사용자인 본인의 내부 웹 브라우저 임프린트 레지스터 영역 안에서 복사 과정이 일어날 뿐 서류 정리함 클라우드 서비스는 그 어떠한 사본 및 데이터 흔적도 수집 또는 열람하지 못함을 엄정 약속합니다.',
    faqList: [
      {
